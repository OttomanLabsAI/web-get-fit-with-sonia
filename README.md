...
- **Gallery layout.** The gallery is a centred flow: rows share a height, each photo keeps its own proportions and width, and rows centre so the edges need not align. Add `class="feature"` to a list item for a larger tile. Portrait originals will slot in tall beside landscape ones.
